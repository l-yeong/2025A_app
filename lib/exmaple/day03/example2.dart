...
class GoodsWidgetState extends State<GoodsWidget>{
  ...
  void goodsSave () async {
    ...
    try{
      ...
      // (web) 빌드시 : localhost:8080 가능 , (mobile) : localhost:8080 만 가능
      // 스프링 컨트롤러 에서  @CrossOrigin(origins = "*") 또는 corsConfig
      final response = await dio.post("http://192.168.1.73:8080/api/goods" , data : obj );
  ...
  void goodsAll() async{
    try{
      final dio = Dio();
      final response = await dio.get("http://192.168.1.73:8080/api/goods/findall");
